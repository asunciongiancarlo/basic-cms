...
class ListOfCategory extends Eloquent {
	...
	public static function categories($active_parent)
	{
		$parent_categories = DB::table('categories')
									 ...
									 ->get();
		 $active = '';
		 $cat    = "<div class='list-group'>";
		 $cat   .= "<b>".link_to('lists_of_categories','All Categories', array('class'=>'list-group-item'))."</b>";
		 foreach ($parent_categories as $parent_category) 
		 {
		 	$active = ($active_parent==$parent_category->id) ? 'active' : '';
		 	$cat   .= link_to('lists_of_categories/show/'.$parent_category->id, '  '.$parent_category->category_name, array('class'=>'list-group-item '.$active));
